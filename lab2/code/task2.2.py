...
for i in range(matrix.shape[0]):
    for j in range(matrix.shape[1]):
        if matrix[i][j] > a:
            ...
            y.append(j)
            z.append(matrix[i][j])
np.savez(r"..\result\result2.2.1.npz", x=x, y=y, z=z)
np.savez_compressed(r"..\result\result2.2.2.npz", x=x, y=y, z=z)
print('Метод savez сохранил результат в result.2.2.1.npz\nМетод savez_compressed сохранил результат в result.2.2.2.npz')

first_size = os.path.getsize(r"..\result\result2.2.1.npz")
second_size = os.path.getsize(r"..\result\result2.2.2.npz")
