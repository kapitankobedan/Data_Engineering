...
def write_to_files(average, maximum, minimum, filter):
    with open(r"C:\Users\Данила\PycharmProjects\DE_practice1\lab1\result\fourth_task_result.txt", "w", encoding="utf-8") as file:
        file.write(f"{average}\n{maximum}\n{minimum}\n")
    with open(r"C:\Users\Данила\PycharmProjects\DE_practice1\lab1\result\fourth_task_result.csv", "w", encoding="utf-8", newline="") as file:
        writer = csv.DictWriter(file, filter[0].keys())
        writer.writeheader()
        for row in filter:
            writer.writerow(row)
data = read_csv(path=r"C:\Users\Данила\PycharmProjects\DE_practice1\lab1\data\fourth_task.txt")
...
print('Среднее, максимум и минимум записаны в файл fourth_task_result.txt')
print('Отфильтрованнфе данные записаны в fourth_task_result.csv')